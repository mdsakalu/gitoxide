...
fn resolve_revspec(
    ...
    match result {
        Err(err) => {
            ...
            let not_found = err.downcast_any_ref::<gix::refs::store::find::existing::Error>();
            if let Some(gix::refs::store::find::existing::Error::NotFound { name }) = not_found {
                let root = repo.workdir().map(ToOwned::to_owned);
                let name: &gix::refs::PartialNameRef = name.as_ref();
