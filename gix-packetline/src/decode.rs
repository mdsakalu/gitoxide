...
pub(crate) fn data_length_limit_exceeded(length_in_bytes: usize) -> Error {
    Error::new(format!(
        "The data received claims to be larger than the maximum allowed size: got {length_in_bytes}, exceeds {MAX_DATA_LEN}"
    ))
}

pub(crate) fn not_enough_data(bytes_needed: usize) -> Error {
    Error::new(format!(
...
pub fn hex_prefix(four_bytes: &[u8]) -> Result<PacketLineOrWantedSize<'_>, Error> {
    ...
    if wanted_bytes == 3 {
        return Err(Error::new("Received an invalid line of length 3"));
    }
    if wanted_bytes == 4 {
        return Err(Error::new("Received an invalid empty line"));
    }
    debug_assert!(
