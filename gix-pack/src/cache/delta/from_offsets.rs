...
impl<T> Tree<T> {
    ...
    pub fn from_offsets_in_pack(
        ...
        let anticipated_num_objects = data_sorted_by_offsets
            ...
            .unwrap_or_default();
        let mut tree = Tree::with_capacity(anticipated_num_objects).or_erased()?;
        ...
        for (idx, data) in data_sorted_by_offsets.enumerate() {
            ...
            match entry.header {
                Tree | Blob | Commit | Tag => {
                    tree.add_root(pack_offset, data).or_erased()?;
                }
                RefDelta { base_id } => {
                    ...
                    tree.add_child(base_pack_offset, pack_offset, data).or_erased()?;
                }
